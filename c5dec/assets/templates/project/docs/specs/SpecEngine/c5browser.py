...
# Kept as a static fallback; at runtime the list is replaced by discover_doc_types().
...
def _parse_yaml_block(raw: str, out: dict) -> None:
    ...
    for line in raw.splitlines():
        # List continuation for multi-line values (links)
...
def discover_doc_types(specs_dir: Path) -> list:
    """
    ...
    if not _YAML:
        print(
            ...
            "falling back to built-in DOC_TYPES.",
            file=sys.stderr,
        )
        return []
    ...
    for pfx in ordered:
        ...
        for key in info["col_keys"]:
            stype = "num" if key in _NUMERIC_FIELDS else "str"
...
def _cell_defect(value: str) -> str:
    css = _DEFECT_BADGE_CSS.get(value, "bg-secondary")
    label = _DEFECT_LABELS.get(value, _esc(value))
...
def _render_cell(key: str, value: str, named: bool) -> str:
    if key == "uid":
...
def _build_table(prefix: str, cols: list, items: list) -> str:
    ...
    for item in items:
        ...
        for key, _label, stype in cols:
            raw = item.get(key, "")
            cell_html = _render_cell(key, str(raw), named)
            if stype == "num":
...
def build_html(items_by_prefix: dict, doc_types: list) -> str:
    import datetime
    ...
    for i, (prefix, _, cols) in enumerate(doc_types):
        ...
        table_html = _build_table(prefix, cols, items)
        panes.append(
...
def main():
    ...
    # built-in DOC_TYPES list when pyyaml is unavailable.
    active_doc_types = discover_doc_types(specs_dir)
    if active_doc_types:
        print(f"Discovered {len(active_doc_types)} document type(s) from .doorstop.yml files.")
    else:
        print("Using built-in DOC_TYPES (pyyaml unavailable or no .doorstop.yml found).")
    ...
    html = build_html(items_by_prefix, active_doc_types)
